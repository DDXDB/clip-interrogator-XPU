...
class Config:
    ...
    device: str = (
        "mps" if torch.backends.mps.is_available() else "xpu" if torch.xpu.is_available() else "cuda" if torch.cuda.is_available() else "cpu")
...
class Interrogator():
    def __init__(self, config: Config):
        ...
        self.dtype = torch.float16 if self.device == 'xpu' or self.device == 'cuda' else torch.float32
    ...
    def load_clip_model(self):
        ...
        if config.clip_model is None:
            ...
            self.clip_model, _, self.clip_preprocess = open_clip.create_model_and_transforms(
                ...
                precision='fp16' if config.device == 'xpu' or config.device == 'cuda' else 'fp32',
